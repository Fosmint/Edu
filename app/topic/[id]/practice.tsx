  ...
  async function handleCheck() {
    ...
    try {
      ...
      try {
        ...
        // Побочные эффекты (БД, setTopic) — вне updater'а setStreak: React может
        // вызвать updater повторно, и сложность изменилась бы дважды.
        const currentTier = topic.current_difficulty_tier;
        const newStreak = check.is_correct ? streak + 1 : streak - 1;
        ...
        if (newStreak >= 3 && currentTier < 4) {
          ...
          setStreak(0);
        } else if (newStreak <= -2 && currentTier > 1) {
          db.runSync(`UPDATE topic_progress SET current_difficulty_tier = ? WHERE topic_id = ?`, [
            currentTier - 1,
            topic.id,
          ]);
