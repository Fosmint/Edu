...
export default function TopicBossScreen() {
  ...
  async function handleSubmitAnswer() {
    ...
    try {
      ...
      // Слабые точки считаем локально: setWeakPoints ниже не успел бы примениться
      // до finishBoss, и последняя ошибка выпала бы из weak_points_json
      const updatedWeakPoints =
        !check.is_correct && check.mistake_type ? [...weakPoints, check.mistake_type] : weakPoints;
      ...
      if (currentIndex + 1 >= questions.length) {
        finishBoss([...answersGiven, check.is_correct], updatedWeakPoints);
      } else {
  ...
  function finishBoss(finalAnswers: boolean[], finalWeakPoints: string[]) {
    if (!topic) return;
    const correctCount = finalAnswers.filter(Boolean).length;
    const scorePct = (correctCount / finalAnswers.length) * 100;

    const db = getDb();
    // score хранится на шкале 0..1 — на ту же шкалу ожидают достижения (score >= 0.7)
    db.runSync(
      `INSERT INTO exams (topic_id, subject_id, type, score, total_questions, correct_answers, weak_points_json)
       VALUES (?, ?, 'boss', ?, ?, ?, ?)`,
      [topic.id, topic.subject_id, scorePct / 100, finalAnswers.length, correctCount, JSON.stringify(finalWeakPoints)]
    );
