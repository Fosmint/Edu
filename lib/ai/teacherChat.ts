...
/** Специальная функция для кнопки "Я нихуя не понял" — переформулирует последнее объяснение */
export async function requestSimplerExplanation(params: {
  ...
  return sendMessageToTeacher({
    ...
    sessionType: "chat",
    userMessage:
      "🤯 Я нихуя не понял. Объясни ещё раз, но по-другому: проще, короче, через понятную аналогию или бытовой пример. Не повторяй предыдущее объяснение теми же словами.",
  });
}
